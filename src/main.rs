...
fn view_at<'a>(mut server: Pin<&'a mut Server>, views: &[ViewData], x: i32, y: i32) -> Option<(Index, SurfaceHit<'a>)> {
    for v in views.iter().rev() {
        let view = server.as_mut().get_view(v.view);
        let rel_x = (x - v.rect.x) as f64;
        let rel_y = (y - v.rect.y) as f64;
        if let Some(hit) = view.surface_at(rel_x, rel_y) {
            return Some((v.view, hit));
        }
    }
    None
}
        ...
        for e in ctx.as_mut().poll_events() {
            match e {
                ...
                Event::XdgSurfaceNew { view } => {
                    let v = ctx.as_mut().get_view(view);
                    let rect = v.get_rect();
                    views.push(ViewData {
                        view,
                        rect,
